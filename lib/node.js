...
Node.prototype.emit = function(message, payload)
{
	if(this._socket)
	{
		try {
			...
			console.sstats('wsc', 'Socket emitted message:', chalk.reset.cyan(message));
			//console.success('wsc', payload);
		}
		catch (err) {
			console.error('wsc', 'Socket emitted error:', err);
		}
	}
    console.success('wsc', 'emit done');
}
...
Node.prototype.validateLatestBlock = function (error, result, timeString)
{
	...
	if(this.stats.block.number - this._lastBlock > 1)
	{
		...
		if( this._latestQueue !== null && this._latestQueue.idle() )
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		function (err, results)
		{
			...
			if(results.peers !== null)
			{
				...
				self.stats.syncing = false; // short-circuit
...
{
	console.stats("wsc", "XXX: NOT sending pending update");
...
Node.prototype.setWatches = function()
{
	...
	web3.eth.isSyncing(function(error, sync) {
		if(!error) {
			if(sync === true) {
				web3.reset(true);
				console.info("SYNC STARTED (XXX: sync state can't be displayed)");
			} else if(sync) {
				...
				// }
				console.info("SYNC UPDATE (XXX: sync state can't be displayed)");
			} else {
				console.info("SYNC STOPPED (XXX: sync state can't be displayed)");
				self.stats.syncing = false;
				//self.setFilters();
			}
		} else {
